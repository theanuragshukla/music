...
var art = artEng;
var quality = 44;
var currentCategory = "english";
var list = function(id, name, author) {
    var artSrc = (art.find(x => x.name === songs[id].name + "_192").url);

    return `<div class="cardlist" id ="cardlist${id}" onclick="playselected(${id})"> 
...
function getIndex(i) {
    var index = data.map(function(name) {
        return name.name;
    }).indexOf(i);
    return index;
}
